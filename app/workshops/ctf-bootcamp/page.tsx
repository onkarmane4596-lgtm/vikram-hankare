...
export default function CTFBootcampWorkshopPage() {
  const [formData, setFormData] = useState({
    fullName: "",
    email: "",
    phone: "",
    teamSize: "Individual / Solo Player",
    message: ""
  });
  ...
  const ctfCategories = [
    "Web Exploitation (SQLi, XSS, SSRF, IDOR, JWT & Logic Bypasses)",
    "Reverse Engineering & Binary Analysis (GDB, Ghidra, Radare2)",
    "Cryptography & Hash Cracking (RSA, AES, Custom Ciphers, Hashcat)",
    "Digital Forensics & PCAP Network Packet Analysis (Wireshark, Memory Dumps)",
    "Steganography & Hidden Data Carving (LST, Audio & Image Metadata)",
    "Privilege Escalation & Active Directory Attack Chains"
  ];
    ...
    <div className="min-h-screen bg-[#020617] text-slate-300 font-sans selection:bg-[#00d4ff]/30 selection:text-white pb-24 overflow-x-hidden relative">
      ...
      <section className="relative pt-32 pb-20 lg:pt-44 lg:pb-28 overflow-hidden border-b border-white/[0.06]">
        <div className="absolute inset-0 z-0 overflow-hidden pointer-events-none">
          <img
            src="/design/5.png"
            alt="Capture The Flag CTF Bootcamp Background"
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="flex items-center gap-2 text-xs text-slate-400 mb-8">
            ...
            <ChevronRight className="w-3 h-3 text-slate-600" />
            <span className="text-[#00d4ff] font-medium">CTF Bootcamp</span>
          </motion.div>
          ...
          <motion.div initial="hidden" animate="visible" variants={staggerContainer} className="max-w-4xl">
            <motion.div variants={fadeUp} className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-[#00d4ff]/10 border border-[#00d4ff]/25 text-xs font-bold text-[#00d4ff] uppercase tracking-widest mb-6 backdrop-blur-md">
              <Flag className="w-3.5 h-3.5 text-[#00d4ff]" /> Competitive Cyber Range Bootcamp
            </motion.div>
            ...
            <motion.h1 variants={fadeUp} className="text-3xl md:text-5xl lg:text-6xl font-extrabold tracking-tight text-white mb-6 leading-[1.15]">
              Capture The Flag <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#00d4ff] via-[#38BDF8] to-white">(CTF) Bootcamp</span>
            </motion.h1>

            <motion.p variants={fadeUp} className="text-base md:text-lg text-slate-300 leading-relaxed mb-8">
              Test your offensive and defensive hacking skills in an intensive 1-3 day Capture The Flag (CTF) bootcamp featuring Jeopardy and Attack-Defense challenges.
            </motion.p>
          </motion.div>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-14">
            <h2 className="text-3xl md:text-5xl font-bold text-white mb-4">
              CTF Challenge <span className="text-[#00d4ff]">Categories</span>
            </h2>
          </div>

          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
            {ctfCategories.map((item, idx) => (
